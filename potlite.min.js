 ...
 */
(function(c,g){var o={VERSION:"1.26",TYPE:"lite"},i=Array.prototype.push,s=Array.prototype.slice,f=Array.prototype.concat,w=Object.prototype.toString,m=Object.prototype.hasOwnProperty,p=/([-.*+?^${}()|[\]\/\\])/g,y=/^(?:[a-zA-Z0-9_~.-]|%[0-9a-fA-F]{2})*$/,n=/List|Collection/i,e=/^[\s\u00A0\u3000]+|[\s\u00A0\u3000]+$/g,q,v,k,h;
...
}}})}return D}});function j(){var F=arguments,E=F.length,G=1,J,I,D;if(E===G){J=this||{};
G--}else{J=F[G-1]}if(J){do{D=F[G];if(D){for(I in D){try{J[I]=D[I]}catch(H){}}}}while(++G<E)
}return J}function B(F,E){var K,H,D,I=arguments.callee,G;if(I.canNodeList==null){if(o.System.currentDocument){try{G=s.call(o.System.currentDocument.documentElement.childNodes)[0].nodeType;
...
}})})})();j(o.Serializer,{serializeToQueryString:function(F){var D=[],E;if(!F||F==false){return""
}if(typeof Buffer!=="undefined"&&F.constructor===Buffer){return F}if(o.isString(F)){return u(F)
}if(o.isObject(F)||o.isArrayLike(F)){E=o.URI.urlEncode;d(F,function(H,G){var L,K,N,I,J=true;
L=o.isArray(H)?H:[G,H];try{K=u(L[0],false);N=L[1]}catch(M){J=false}if(J&&(K||N)){I=K?"=":"";
if(o.isArray(N)){K=u(K,true)+"[]"}else{N=[N]}d(N,function(O){D[D.length]=E(K)+I+E(u(O,true))
})}})}return D.join("&")},parseFromQueryString:function(I,H){var D=[],G,F,E;if(o.isObject(I)||o.isArray(I)){return I
...
}else{D[K]=[N]}}else{D[D.length]=[J,N]}}else{if(H){D[K]=N}else{D[D.length]=[K,N]}}}})
...
});this.handleCache=[]},initEvents:function(){var ae=this,ac=false,af=this.target,ad,ab=this.handleCache,ah=this.options,ag=o.Signal;
ab[ab.length]=ag.attach(af,"drop",function(al){var ak,ai,aj=0;ae.isShow=false;ak=al.dataTransfer&&al.dataTransfer.files;
if(ak){if(ah.onDrop){ah.onDrop.call(ae,ak)}ai=new FileReader();ai.onloadend=function(am){aj--;
if(am&&am.target&&am.target.result!=null){ae.loadedFiles.push(am.target.result);if(aj<=0){if(ah.onLoadEnd){ah.onLoadEnd.call(ae)
}}}};d(ak,function(ao){var am,an;if(ao){aj++;an=ao.size;am=ao.name;ai.readAsDataURL(ao);
if(ae.isImageFile(ao.type)){ae.loadAsImage(ao,am,an)}else{if(ae.isTextFile(ao.type)){ae.loadAsText(ao,am,an)
}else{ae.loadAsUnknown(ao,am,an)}}}})}});ab[ab.length]=ag.attach(af,"dragenter",function(ai){ae.isShow=true;
ag.cancelEvent(ai)});ab[ab.length]=ag.attach(af,"dragover",function(ai){ae.isShow=true;
ag.cancelEvent(ai)});ab[ab.length]=ag.attach(af,"dragleave",function(ai){ae.isShow=false
});if(ah.onHide){ah.onHide.call(ae)}ad=o.currentDocument().documentElement;ab[ab.length]=ag.attach(ad,"drop",function(ai){ae.isShow=false;
if(ah.onHide){ah.onHide.call(ae)}ag.cancelEvent(ai)});ab[ab.length]=ag.attach(ad,"dragleave",function(ai){o.Internal.setTimeout(function(){if(ae.isShow){ae.isShow=false
}else{if(ah.onHide){ah.onHide.call(ae)}}},1000)});d(["dragenter","dragover"],function(ai){ab[ab.length]=ag.attach(ad,ai,function(am){var al=am&&am.dataTransfer,ak,aj;
if(al){if(al.files&&al.files.length){ak=true}else{if(al.types){aj=/Files/i;if(aj.test(al.types)){ak=true
}else{if(o.isArrayLike(al.types)){d(al.types,function(an){if(aj.test(an)){ak=true;
throw o.StopIteration}})}}}}}if(ak){ae.isShow=true;if(ah.onShow){ah.onShow.call(ae)
}}ag.cancelEvent(am)})})},isImageFile:function(ab){return/image/i.test(ab)},isTextFile:function(ab){return !/image|audio|video|zip|compress/i.test(ab)
...
ag=o.Net.request(ae,ab)}return o.Deferred.maybeDeferred(ag)},loadAsImage:function(ae,ac,ad){var ab=new FileReader(),af=this.options.onLoadImage;
ab.onload=function(ag){if(af){af.call(this,ag&&ag.target&&ag.target.result,ac,ad)
}};ab.readAsDataURL(ae)},loadAsText:function(ae,ac,ad){var ab=new FileReader(),af=this.options.onLoadText;
ab.onload=function(ag){if(af){af.call(this,ag&&ag.target&&ag.target.result,ac,ad)
}};ab.readAsText(ae,this.encoding)},loadAsUnknown:function(ae,ac,ad){var ab=new FileReader(),af=this.options.onLoadUnknown;
ab.onload=function(ag){if(af){af.call(this,ag&&ag.target&&ag.target.result,ac,ad)
