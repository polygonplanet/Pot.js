 ...
 */
(function(c,g){var m={VERSION:"1.12",TYPE:"lite"},q=Array.prototype.slice,f=Array.prototype.concat,u=Object.prototype.toString,l=/List|Collection/i,e=/^[\s\u00A0\u3000]+|[\s\u00A0\u3000]+$/g,n=/([-.*+?^${}()|[\]\/\\])/g,w=/^(?:[a-zA-Z0-9_~.-]|%[0-9a-fA-F]{2})*$/,o,t,i,h;
...
}else{if(!E){C=D?1:""}}break;case"object":if(D){F=D.constructor;if(F===String||F===Number||(typeof XML!=="undefined"&&F===XML)){C=D
...
d(m.Deferred.states,function(M,L){m.Deferred.states[M]=L});j(m.Deferred,{defaults:{speed:m.Deferred.speeds.ninja,canceller:null,stopper:null,async:true}});
m.Deferred.fn=m.Deferred.prototype=j(m.Deferred.prototype,{constructor:m.Deferred,id:m.Internal.getMagicNumber(),serial:null,chains:[],chained:false,cancelled:false,freezing:false,tilling:false,waiting:false,nested:0,state:null,results:null,destassign:false,options:{},plugins:{},NAME:"Deferred",toString:m.toString,isDeferred:m.isDeferred,init:function(L){if(!this.serial){this.serial=k(this)
}this.options={};this.plugins={};B.call(this,z(L),m.Deferred.defaults);j(this,{results:{success:null,failure:null},state:m.Deferred.states.unfired,chains:[],nested:0,chained:false,cancelled:false,freezing:false,tilling:false,waiting:false,destassign:false});
m.Internal.referSpeeds.call(this,m.Deferred.speeds);return this},speed:function(O){var M=this,L=arguments,N;
if(m.isNumeric(O)){N=O-0}else{if(m.isNumeric(m.Deferred.speeds[O])){N=m.Deferred.speeds[O]-0
}else{N=this.options.speed}}if(this.state===m.Deferred.states.unfired&&!this.chains.length){if(L.length===0){return this.options.speed
}this.options.speed=N}else{this.then(function(P){if(L.length===0){return M.options.speed
}M.options.speed=N;return P})}return this},async:function(N){var M=this,L=arguments;
if(this.state===m.Deferred.states.unfired&&!this.chains.length){if(L.length===0){return this.options.async
}this.options.async=!!N}else{this.then(function(O){if(L.length===0){return M.options.async
}M.options.async=!!N;return O})}return this},canceller:function(M){var L=arguments;
if(this.state===m.Deferred.states.unfired&&!this.chains.length){if(L.length===0){return this.options.cancellers
}if(!this.cancelled&&m.isFunction(M)){this.options.cancellers.push(M)}}else{this.stopper.apply(this,L)
}return this},stopper:function(N){var M=this,L=arguments;if(this.state===m.Deferred.states.unfired&&!this.chains.length){this.canceller.apply(this,L)
}else{this.then(function(O){if(L.length===0){return M.options.stoppers}if(!M.cancelled&&m.isFunction(N)){M.options.stoppers.push(N)
}return O})}return this},then:function(M,L){if(!this.chained&&!this.cancelled){this.chains.push({success:M,failure:L});
if(this.state&m.Deferred.states.fired){if(!this.freezing&&!this.tilling&&!this.waiting){E.call(this)
}}}m.Internal.referSpeeds.call(this,m.Deferred.speeds);return this},rescue:function(L){return this.then(null,L)
},ensure:function(L){return this.then(L,L)},cancel:function(){if(!this.cancelled){this.cancelled=true;
switch(this.state){case m.Deferred.states.unfired:G.call(this,"cancellers");if(this.state===m.Deferred.states.unfired){this.raise(new Error(this))
}break;case m.Deferred.states.success:G.call(this,"stoppers");if(m.isDeferred(this.results.success)){this.results.success.cancel()
}break;case m.Deferred.states.failure:G.call(this,"stoppers");break;default:break
}}return this},begin:function(){var N=this,L,M=z(arguments),O;L=M[0];if(M.length>1){O=M
}else{O=M[0]}if(!this.cancelled&&this.state===m.Deferred.states.unfired){if(m.isDeferred(L)&&!L.cancelled){L.ensure(function(){N.begin.apply(this,arguments)
})}else{this.options.cancellers=[];K.call(this,O)}}m.Internal.referSpeeds.call(this,m.Deferred.speeds);
return this},raise:function(){var M=z(arguments),L,N;L=M[0];if(!m.isError(L)){M[0]=new Error(L)
}if(M.length>1){N=M}else{N=M[0]}return this.begin.apply(this,z(N))},end:function(){this.chained=true;
return this},wait:function(P,N){var O,M=this,L=arguments;O=new m.Deferred();return this.then(function(Q){if(m.isError(Q)){throw Q
}M.waiting=true;m.Deferred.wait(P).ensure(function(R){M.waiting=false;if(m.isError(R)){O.raise(R)
}else{O.begin(L.length>=2?N:Q)}});return O})},till:function(L){var M=this,N=new m.Deferred();
return this.then(function(O){if(m.isError(O)){throw O}M.tilling=true;m.Deferred.till(L,O).ensure(function(P){M.tilling=false;
if(m.isError(P)){N.raise(P)}else{N.begin(O)}});return N})},args:function(){var M=z(arguments),L=M.length;
if(L===0){return m.Deferred.lastResult(this)}else{return this.then(function(){var O,N=z(arguments);
if(N.length>1){O=N}else{O=N[0]}if(L>1){return M}else{if(m.isFunction(M[0])){return M[0].apply(this,z(O))
}else{return M[0]}}})}},data:function(){var P=this,M=this,N=z(arguments);var O,L=N.length,Q=".";
if(this.options){if(!this.options.storage){this.options.storage={}}switch(L){case 0:M={};
d(this.options.storage,function(T,R){try{if(R&&R.charAt(0)===Q){M[R.substring(1)]=T
}}catch(S){}});break;case 1:if(N[0]==null){this.options.storage={}}else{if(m.isObject(N[0])){d(N[0],function(S,R){P.options.storage[Q+s(R)]=S
})}else{M=this.options.storage[Q+s(N[0])]}}break;case 2:this.options.storage[Q+s(N[0])]=N[1];
break;default:O=0;do{this.options.storage[Q+s(N[O++])]=N[O++]}while(O<L);break}}return M
},update:function(){var M=m.Deferred.fn,L=z(arguments);L.unshift(M);j.apply(M,L);
return this}});m.Deferred.prototype.init.prototype=m.Deferred.prototype;function H(L){if(m.isError(L)){this.state=m.Deferred.states.failure
}else{this.state=m.Deferred.states.success}return this.state}function K(L){H.call(this,L);
this.results[m.Deferred.states[this.state]]=L;if(!this.freezing&&!this.tilling&&!this.waiting){E.call(this)
}}function E(L){if(L||(!this.freezing&&!this.tilling&&!this.waiting)){if(this.options&&this.options.async){C.call(this)
}else{J.call(this)}}}function C(){var L=this,M,N;if(this.options&&m.isNumeric(this.options.speed)){M=this.options.speed
}else{M=m.Deferred.defaults.speed}this.freezing=true;N=function(){try{D.call(L)}catch(O){L.freezing=false;
throw O}if(I.call(L)){E.call(L,true)}else{L.freezing=false}};if(!M&&this.state===m.Deferred.states.unfired){m.Internal.callInBackground.flush(N)
}else{m.Internal.setTimeout(N,M)}}function J(){D.call(this);if(this.options&&this.options.async){E.call(this)
}}function D(){var N=this,L,M,Q,O;L=this.results[m.Deferred.states[this.state]];O=null;
while(I.call(this)){M=this.chains.shift();Q=M&&M[m.Deferred.states[this.state]];if(!m.isFunction(Q)){continue
}try{if(this.destassign||(m.isNumber(Q.length)&&Q.length>1&&m.isArray(L)&&L.length===Q.length)){L=Q.apply(this,L)
}else{L=Q.call(this,L)}this.destassign=false;this.state=H.call({},L);if(m.isDeferred(L)){O=function(R){return F.call(N,R)
};this.nested++}}catch(P){this.destassign=false;this.state=m.Deferred.states.failure;
L=P;if(!m.isError(L)){L=new Error(L)}}if(this.options&&this.options.async){break}}this.results[m.Deferred.states[this.state]]=L;
if(O&&this.nested){L.ensure(O).end()}}function I(){return this.chains&&this.chains.length&&this.nested===0&&!this.cancelled
}function F(L){K.call(this,L);this.nested--;if(this.nested===0&&!this.cancelled&&(this.state&m.Deferred.states.fired)){E.call(this)
}}function B(M,R){var P,Q,L,O,N,S;if(M){if(M.length===1&&M[0]&&m.isObject(M[0])){P=M[0];
if(P.speed!==S||P.canceller!==S||P.async!==S||P.stopper!==S){Q=P.speed;L=P.canceller;
O=P.stopper;N=P.async}else{Q=P.options&&P.options.speed;L=P.options&&P.options.canceller;
O=P.options&&P.options.stopper;N=P.options&&P.options.async}}else{if(M.length===1&&M[0]&&m.isArray(M[0])){P=M[0]
}else{P=M}d(P||[],function(T){if(Q===S&&m.isNumeric(T)){Q=T}else{if(Q===S&&m.isNumeric(m.Deferred.speeds[T])){Q=m.Deferred.speeds[T]
}else{if(L===S&&m.isFunction(T)){L=T}else{if(N===S&&m.isBoolean(T)){N=T}else{if(O===S&&L===S&&m.isFunction(T)){O=T
...
}if(!m.isNumeric(Q)){if(this.options.speed!==S&&m.isNumeric(this.options.speed)){Q=this.options.speed-0
}else{Q=R.speed}}if(!m.isFunction(L)){L=R.canceller}if(!m.isFunction(O)){O=R.stopper
}if(!m.isBoolean(N)){if(this.options.async!==S&&m.isBoolean(this.options.async)){N=this.options.async
}else{N=R.async}}j(this.options,{speed:Q-0,async:N});if(m.isFunction(L)){this.options.cancellers.push(L)
}if(m.isFunction(O)){this.options.stoppers.push(O)}return this}function G(L){var M;
while(this.options[L]&&this.options[L].length){M=this.options[L].shift();if(m.isFunction(M)){M.call(this)
}}}j(m.Deferred,{extendSpeeds:function(R,O,L,N){var Q={},M={};var P=function(S,T){return function(){var W={},U=arguments,V=U.callee;
U=z(U);B.call(W,U,{speed:T,canceller:m.Deferred.defaults.canceller,stopper:m.Deferred.defaults.stopper,async:m.Deferred.defaults.async});
W.speedName=S;U.unshift(W);return L.apply(V.instance,U)}};d(N,function(T,S){M[S]=P(S,T)
});return j(R[O],M)}});j(m.Internal,{referSpeeds:j(function(L){var M=arguments.callee,O,N;
if(L&&this.forEach.fast.instance!==this){for(O in M.props){if(O in this&&this[O]){for(N in M.speeds){if(N in L&&N in this[O]&&this[O][N]){this[O][N].instance=this
...
m.Deferred.extendSpeeds(m,"Deferred",function(L){return m.Deferred(L)},m.Deferred.speeds)
...
}else{return m.Net.XHR.request(I,H)}},XHR:{ReadyState:{UNINITIALIZED:0,LOADING:1,LOADED:2,INTERACTIVE:3,COMPLETE:4},factory:function(){var H,J;
...
};H.prototype=j(H.prototype,{xhr:null,url:null,options:{},deferred:null,doit:function(J,I){this.url=s(J,true);
this.deferred=new m.Deferred();if(this.url){try{this.setOptions(I);if(this.factory()){this.open();
this.setHeaders();this.setReadyStateChange();this.send()}}catch(L){try{this.cancel(true)
}catch(K){}this.deferred.raise(L)}}return this},factory:function(){this.xhr=m.Net.XHR.factory();
...
}catch(J){}}},send:function(){this.xhr.send(this.options.sendContent)}});H.prototype.doit.prototype=H.prototype;
return function(J,I){return(new H(J,I)).deferred}})()},requestByGreasemonkey:function(I,H){var M,K,L,J;
M=new m.Deferred();K=j({},H||{});L={sendContent:"data",mimeType:"overrideMimeType",username:"user"};
d(L,function(N,O){if(O in K){K[N]=K[O]}});K.method=v(K.method).toUpperCase()||"GET";
K.url=B(I,K.queryString);if(K.data){K.data=D(K.data)}if(K.data&&K.method==="GET"){K.method="POST"
...
},onerror:function(){M.raise.apply(M,arguments)}});m.Deferred.callLazy(function(){GM_xmlhttpRequest(K)
});return M},requestByJSONP:(function(){var H={KEY:/json|call/i,DONE:/loaded|complete/};
