 ...
 */
(function(globals,undefined){var Pot={VERSION:"1.19",TYPE:"lite"},slice=Array.prototype.slice,concat=Array.prototype.concat,toString=Object.prototype.toString,hasOwnProperty=Object.prototype.hasOwnProperty,RE_ARRAYLIKE=/List|Collection/i,RE_TRIM=/^[\s\u00A0\u3000]+|[\s\u00A0\u3000]+$/g,RE_RESCAPE=/([-.*+?^${}()|[\]\/\\])/g,RE_PERCENT_ENCODED=/^(?:[a-zA-Z0-9_~.-]|%[0-9a-fA-F]{2})*$/,Ci,Cc,Cr,Cu;
...
})(),isStopIter:function(o){if(Pot.StopIteration!==undefined&&(o==Pot.StopIteration||o instanceof Pot.StopIteration)){return true
}if(typeof StopIteration!=="undefined"&&(o==StopIteration||o instanceof StopIteration)){return true
}if(this&&this.StopIteration!==undefined&&(o==this.StopIteration||o instanceof this.StopIteration)){return true
}if(~toString.call(o).indexOf(SI)||~String(o&&o.toString&&o.toString()||o).indexOf(SI)){return true
}return false},isIterable:function(x){return !!(x&&Pot.isFunction(x.next)&&(~x.next.toString().indexOf(SI)||Pot.isNativeCode(x.next)))
...
result=true}catch(er){}}}}catch(e){}return result},dump:function(o){var r,me=arguments.callee,repr;
repr=function(x){return(x==null)?String(x):x.toString?x.toString():String(x)};if(o==null){return String(o)
}if(typeof uneval==="function"){return uneval(o)}else{if(typeof o.toSource==="function"){return o.toSource()
}}r=[];switch(typeLikeOf(o)){case"array":each(o,function(v){r[r.length]=me(v)});return"["+r.join(", ")+"]";
case"object":if(Pot.isNodeLike(o)||Pot.isWindow(o)||Pot.isDocument(o)){r[r.length]=toString.call(o)
}else{each(o,function(v,k){r[r.length]=k+": "+me(v)})}return"{"+r.join(", ")+"}";
case"string":return'"'+repr(o)+'"';case"function":return"("+repr(o)+")";default:return repr(o)
}},divConsole:update(function(msg){var me=arguments.callee,ie6,doc,de,onResize,onScroll,onClick;
...
wrapper.appendChild(me.ieConsole);doc.body.appendChild(wrapper);me.append()})}}}},{append:function(msg){var me=this,v,s,stack;
if(me.ieConsole){try{stack=arrayize(me.msgStack||[]);while(me.msgStack&&me.msgStack.length){me.msgStack.pop()
}if(arguments.length){stack.push(msg)}while(stack.length){v=stack.shift();s=debug.dump(v);
if(Pot.isString(v)&&s.charAt(0)==='"'&&s.slice(-1)==='"'){s=s.slice(1,-1)}each([document.createTextNode(s),me.hr.cloneNode(false)],function(node){me.ieConsole.appendChild(node)
...
},Math.min(1000,interval+(now()-time)))}else{d.begin()}return d})},parallel:function(deferredList){var result,args=arguments,d,deferreds,values,bounds;
...
this.iter=this.repeat(object,callback,context)}else{if((type&types.items)===types.items){this.result=[];
this.iter=this.items(object,callback,context)}else{if((type&types.zip)===types.zip){this.result=[];
this.iter=this.zip(object,callback,context)}else{if(Pot.isArrayLike(object)){this.result=object;
...
return that.result}}}}},items:function(object,callback,context){var that=this,copy,i=0,value,prop,isPair;
if(Pot.isObject(object)){copy=[];for(prop in object){if(hasOwnProperty.call(object,prop)){try{value=object[prop]
}catch(e){continue}copy[copy.length]=[prop,value]}}isPair=true}else{if(Pot.isArrayLike(object)){copy=arrayize(object)
}}if(!copy||!copy.length){return this.noop()}if(Pot.isFunction(callback)){return{next:function(){var result,c,key,val;
while(true){if(i>=copy.length){throw Pot.StopIteration}if(!(i in copy)){i++;continue
}try{c=copy[i];if(isPair){key=c[0];val=c[1]}else{key=i;val=c}}catch(e){i++;continue
}result=callback.call(context,[key,val],object);i++;that.result[that.result.length]=result;
return result}}}}else{return{next:function(){var r,t,k,v;while(true){if(i>=copy.length){throw Pot.StopIteration
}if(!(i in copy)){i++;continue}try{t=copy[i];if(isPair){k=t[0];v=t[1]}else{k=i;v=t
}}catch(e){i++;continue}i++;r=[k,v];that.result[that.result.length]=r;return r}}}
}},zip:function(object,callback,context){var that=this,copy,i=0,max;if(Pot.isArrayLike(object)){copy=arrayize(object);
max=copy.length}if(!max||!copy||!copy.length){return this.noop()}if(Pot.isFunction(callback)){return{next:function(){var result,zips=[],j,item;
for(j=0;j<max;j++){item=arrayize(copy[j]);if(!item||!item.length||i>=item.length){throw Pot.StopIteration
}zips[zips.length]=item[i]}result=callback.call(context,zips,object);that.result[that.result.length]=result;
i++;return result}}}else{return{next:function(){var z=[],k,t;for(k=0;k<max;k++){t=arrayize(copy[k]);
if(!t||!t.length||i>=t.length){throw Pot.StopIteration}z[z.length]=t[i]}that.result[that.result.length]=z;
i++;return z}}}}});Pot.Internal.LightIterator.prototype.doit.prototype=Pot.Internal.LightIterator.prototype;
...
result=o.result}return result},items:function(object,callback,context){var result=[],that,iter,o;
that=Pot.Internal.LightIterator.fn;if(object){o={noop:that.noop,result:[]};iter=that.items.call(o,object,callback,context);
Pot.Internal.LightIterator.QuickIteration.resolve(iter);result=o.result}return result
},zip:function(object,callback,context){var result=[],that,iter,o;that=Pot.Internal.LightIterator.fn;
if(object){o={noop:that.noop,result:[]};iter=that.zip.call(o,object,callback,context);
Pot.Internal.LightIterator.QuickIteration.resolve(iter);result=o.result}return result
}}});update(Pot.tmp,{createLightIterateConstructor:function(creator){var methods,construct,name;
...
}}}),items:Pot.tmp.createLightIterateConstructor(function(interval){if(Pot.System.isWaitable&&interval<Pot.Internal.LightIterator.speeds.normal){return function(object,callback,context){var opts={};
opts.type=Pot.Internal.LightIterator.types.items;opts.interval=interval;opts.async=false;
opts.context=context;return(new Pot.Internal.LightIterator(object,callback,opts)).result
}}else{return function(object,callback,context){return Pot.Internal.LightIterator.QuickIteration.items(object,callback,context)
}}}),zip:Pot.tmp.createLightIterateConstructor(function(interval){if(Pot.System.isWaitable&&interval<Pot.Internal.LightIterator.speeds.normal){return function(object,callback,context){var opts={};
opts.type=Pot.Internal.LightIterator.types.zip;opts.interval=interval;opts.async=false;
opts.context=context;return(new Pot.Internal.LightIterator(object,callback,opts)).result
}}else{return function(object,callback,context){return Pot.Internal.LightIterator.QuickIteration.zip(object,callback,context)
...
}}),items:Pot.tmp.createLightIterateConstructor(function(interval){return function(object,callback,context){var opts={};
opts.type=Pot.Internal.LightIterator.types.items;opts.interval=interval;opts.async=true;
opts.context=context;return(new Pot.Internal.LightIterator(object,callback,opts)).deferred
}}),zip:Pot.tmp.createLightIterateConstructor(function(interval){return function(object,callback,context){var opts={};
opts.type=Pot.Internal.LightIterator.types.zip;opts.interval=interval;opts.async=true;
opts.context=context;return(new Pot.Internal.LightIterator(object,callback,opts)).deferred
...
}},{NAME:"items",method:Pot.Deferred.items,context:null,speed:true,iterable:Pot.Deferred.items,args:function(arg,args){return[arg].concat(args)
}},{NAME:"zip",method:Pot.Deferred.zip,context:null,speed:true,iterable:Pot.Deferred.zip,args:function(arg,args){return[arg].concat(args)
