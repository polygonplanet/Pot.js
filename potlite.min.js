 ...
 */
(function(c,g){var n={VERSION:"1.22",TYPE:"lite"},r=Array.prototype.slice,f=Array.prototype.concat,v=Object.prototype.toString,l=Object.prototype.hasOwnProperty,m=/List|Collection/i,e=/^[\s\u00A0\u3000]+|[\s\u00A0\u3000]+$/g,o=/([-.*+?^${}()|[\]\/\\])/g,x=/^(?:[a-zA-Z0-9_~.-]|%[0-9a-fA-F]{2})*$/,p,u,i,h;
...
return D.INITIAL_NUMBER+(D.count++)},{count:0,INITIAL_NUMBER:Number("0xC26BEB642C0A")||(2147483647^2047)}),getExportObject:function(G){var F,H,D;
if(G){if(n.System.isNonBrowser){F=n.Global||c}else{F=(n.isWindow(c)&&c)||(n.isWindow(n.Global)&&n.Global)||n.currentWindow()
}if(!F&&typeof window!=="undefined"&&n.isWindow(window)){F=window}if(F){do{H=k(n,"")
}while(H in F);F[H]=1;D=(new Function("try{return "+H+"===1;}catch(e){}"))();try{delete F[H]
}catch(E){try{F[H]=(void 0)}catch(E){}}if(!D){F=n.Global}}}if(!F){if(n.System.isNodeJS){if(typeof module==="object"&&typeof module.exports==="object"){F=module.exports
}else{if(typeof exports==="object"){F=exports}else{F=c}}}else{F=c}if(!F){F=c||n.Global||n.currentWindow()
}}return F}},update:j,Pot:n})})(typeof navigator!=="undefined"&&navigator||{});j(n.System,(function(){var E={},C;
...
n.update({globalEval:j(function(G){var F=arguments.callee,J,D,E,I,C;if(G&&F.patterns.valid.test(G)){if(n.System.hasActiveXObject){if(typeof execScript!=="undefined"&&execScript&&F.test(execScript)){return execScript(G,F.language)
...
}}if(n.System.isGreasemonkey){return n.localEval(G,D||n.Global)}if(D){if(D[E].call&&D[E].apply&&F.test(E,D,true)){return D[E].call(D,G)
}if(F.worksForGlobal==null){F.worksForGlobal=false;do{J=k(n,"")}while(J in D);D[E]("var "+J+"=1;");
if(J in D&&D[J]===1){F.worksForGlobal=true}try{delete D[J]}catch(H){try{D[J]=(void 0)
}catch(H){}}}if(F.worksForGlobal){return D[E](G)}}if(n.System.isWebBrowser&&typeof document==="object"&&document.body){I=document;
C=I.createElement("script");C.type="text/javascript";C.defer=false;C.appendChild(I.createTextNode(G));
I.body.appendChild(C);I.body.removeChild(C)}else{return n.localEval(G,D||n.Global)
}}},{language:"JavaScript",patterns:{valid:/\S/},test:function(E,G,D){var C=false,H="(void 0);";
try{if(G){if(D){G.func.call(null,H)}else{G.func(H)}}else{if(D){E.call(null,H)}else{E(H)
}}C=true}catch(F){C=false}return C}}),localEval:j(function(H,D){var G=n.globalEval,F=arguments.callee,E,C;
if(H&&G.patterns.valid.test(H)){E="eval";if(E in c&&G.test(E,c)){C=c}else{if(E in n.Global&&G.test(E,n.Global)){C=n.Global
}}if(C&&C[E]){if(C[E].call&&C[E].apply&&G.test(E,C,true)){return F.doEval(H,C,D)}else{if(D==null){return F.doEval(H,C,D,true)
}}}return F.doEvalByFunc(H,D)}},{isLiteral:/^\s*(?!(?:return|var|if|do|for|try|while)\b\s*)[\w$.!"'~(){}[\]]/,isFunc:/^\s*function\b[^{]*[{][\s\S]*[}][^}]*$/,clean:/^(?:[{[(']{0}[')\]}]+|)[;\s\u00A0]*|[;\s\u00A0]*$/g,doEval:function(){try{return arguments[1]["eval"].call(arguments[2]||arguments[1],arguments[0])
}catch(C){}if(arguments[3]){return arguments[1]["eval"](arguments[0])}else{return n.localEval.doEvalByFunc(arguments[0],arguments[2]||arguments[1])
}},doEvalByFunc:function(E,C){var D=n.localEval,F;if(D.isFunc.test(E)||(D.isLiteral.test(E)&&!n.hasReturn(E))){F="return("+String(E).replace(D.clean,"")+");"
}else{F=E}return(new Function("return(function(){"+F+"}).call(this);")).call(C)}}),hasReturn:(function(){var D={STRIP:new RegExp("^\\s*function\\b[^{]*[{]|[}][^}]*$|/[*][\\s\\S]*?[*]/|/{2,}[^\\r\\n]*(?:\\r\\n|\\r|\\n|)|\"(?:\\\\.|[^\"\\\\])*\"|'(?:\\\\.|[^'\\\\])*'","g"),RETURN:/(?:^|\s|[^\w$.]\b)return(?:[^\w$.]\b|\s|$)/,FUNC:/(?:^|\s|[^\w$.]\b)function(?:[^\w$.]\b|\s|)[^{}]*$/,PREREGEX:/(?:^|[,;:!?=&|!([]|[^\w$.<>%'"@){}\]])\s*\/$/,REGEXP:/(\/(?![*])(?:\\.|[^\/\r\n\\])+\/)$/,E4X:/(?:^|[(){}<>&|%*~^!?:;,\/[\]=+-])\s*<([^\s>]*)[^>]*>$/,TAG:/<([^\s>]*)[^>]*>$/},C=8192,F=0,E={};
return function(J){var M=false,H,N,P,G,O,U,V,W,R,L,K,Q,T,I,X,S;H=t(J&&J.toString&&J.toString()||String(J));
...
}}break;case"]":case"<":if((I||!S)&&R===0){if(L>0){K+=W}else{O+=W}}break;case">":if(R===0){if(L===0){O+=W;
if(D.E4X.test(O)){T=O.match(D.TAG);L++;O=O.slice(0,-T[0].length)+" ";X=T[1];P=new RegExp("</"+y(X)+">$");
N=new RegExp("<"+y(X)+"\\b[^>]*>$")}}else{if(L>0){if(S){if((!I&&K.slice(-2)==="--")||(I&&K.slice(-2)==="]]")){S=I=false;
...
}}if(F<C){E[G]=M;F++}return M}})(),override:function(C,E,H,G){var F=[],D;if(C&&E&&(H||G)){if(G){if(n.isFunction(H)){D=H;
H=G;G=D}}else{if(n.isFunction(H)){G=H}}if(n.isRegExp(H)){H=[H,""]}else{if(n.isString(H)){H=[new RegExp(y(H),"g"),""]
}}d(A(E),function(I){if(I){d(C,function(J,K){if(n.isFunction(J)&&((n.isRegExp(I)&&I.test(K))||I==K)){F[F.length]=K
}})}});d(F,function(N){var M,K,L,J,I;if(l.call(C,N)){M=C[N];I=!!(C[N]&&C[N].apply&&typeof C[N].apply===typeof C[N].call);
if(H){K=L=M.toString();J=A(H);if(!n.isArray(J[0])){J=[J]}d(J,function(O){var R,Q;
try{R=O[0];Q=O[1];if(n.isString(R)){R=new RegExp(y(R),"g")}if(!n.isString(Q)){Q=""
}K=K.replace(R,Q)}catch(P){}});if(L!==K){M=n.localEval(K,C)}}C[N]=j(function(){var P=this,O=arguments;
if(n.isFunction(G)){return G.call(P,function(){var Q=arguments;if(Q.length===1&&Q[0]&&Q[0].callee&&Q[0].callee===O.callee){if(I){return M.apply(P,A(Q[0]))
}else{return B(C,N,A(Q[0]))}}else{if(I){return M.apply(P,A(Q))}else{return B(C,N,A(Q))
}}},O,P,N)}else{return M.apply(P,O)}},C[N]);if(!C[N].overridden||n.isNumber(C[N].overridden)){j(C[N],{overridden:((C[N].overridden||0)-0)+1})
}}})}return C}});function j(){var E=arguments,D=E.length,F=1,I,H,C;if(D===F){I=this||{};
F--}else{I=E[F-1]}if(I){do{C=E[F];if(C){for(H in C){try{I[H]=C[H]}catch(G){}}}}while(++F<D)
}return I}function A(E,D){var J,G,C,H=arguments.callee,F;if(H.canNodeList==null){if(n.System.currentDocument){try{F=r.call(n.System.currentDocument.documentElement.childNodes)[0].nodeType;
...
}else{S=X.call(this,Q)}if(S===g&&this.state!==n.Deferred.states.failure&&!n.isError(Q)&&!n.hasReturn(X)){S=Q
}Q=S;this.destAssign=false;this.state=K.call({},Q);if(n.isDeferred(Q)){V=function(Y){return I.call(U,Y)
};this.nested++}}catch(W){Q=W;if(n.isStopIter(Q)){R=true}else{D.call(this,Q)}this.destAssign=false;
...
I=J[1];K=function(){var L={};L.inputs=arguments;return this.then(function(){L.results=arguments;
return I.call(this,L)})};H[G]=K;j(F,H);C++}})}return C},unregister:function(){var C,E=n.Deferred.fn,D=A(arguments),F;
C=0;if(D.length>1){F=D}else{F=D[0]}d(A(F),function(G){try{delete E[G];C++}catch(H){}});
return C},deferrize:function(D,I){var C=arguments,G,E,F;try{switch(C.length){case 0:throw false;
case 1:G=D;break;case 2:default:if(n.isObject(I)){E=I;G=D}else{G=I;E=D}break}if(!G){throw G
}}catch(H){F=H;throw (n.isError(F)?F:new Error(F))}return function(){var K=this,J=A(arguments),L=new n.Deferred();
L.then(function(){var N,M,R=[],O=false,P,S;S=n.Deferred.isFired;N=new n.Deferred();
d(J,function(T){if(n.isFunction(T)){R.push(function(){var U,W;try{U=T.apply(K,arguments)
}catch(V){W=V;if(!S(N)){N.raise(W)}}finally{if(!S(N)){N.begin(U)}}if(W!=null){throw W
}return U});O=true}else{R[R.length]=T}});try{M=B(E,G,R)}catch(Q){P=Q;if(!O&&!S(N)){N.raise(P)
}}finally{if(!O&&!S(N)){N.begin(M)}}if(P!=null){throw n.isError(P)?P:new Error(P)
...
}F[F.length]=K;I.then(function(){if(L.state===n.Deferred.states.unfired){n.Deferred.flush(L)
}L.then(function(M){if(F.length){D[K]=M;F.pop();if(F.length===0){C.begin(D)}}},function(M){F=[];
C.raise(M)})})});n.Deferred.flush(I)}return C},chain:function(){var D=arguments,C=D.length,G,E,F;
...
if(!this.async&&this.waiting===true&&n.System.isWaitable){n.XPCOM.throughout(function(){return C.waiting!==true
})}},execute:function(C,F){var E,D=this;this.waiting=true;if(!C){this.result={};this.waiting=false
...
C=H.result}return C}}});j(n.tmp,{createLightIterateConstructor:function(G){var D,C,E,F=function(I){var H;
if(n.Internal.LightIterator.speeds[I]===g){H=n.Internal.LightIterator.defaults.speed
...
}else{L.begin(M)}});return L})},n.Internal.LightIterator.speeds)}})},createSyncIterator:function(F){var D,C,E=function(H){var G=H;
if(!G){d(n.Internal.LightIterator.speeds,function(J,I){if(J===n.Internal.LightIterator.defaults.speed){G=I;
...
(function(){n.update({Signal:{}});j(n.Signal,{Handler:j(function(J){return new n.Signal.Handler.prototype.init(J)
},{advices:{normal:1,before:2,around:4,after:8,propBefore:16,propAround:32,propAfter:64},beforeTrapper:function(L,K,J){n.Signal.signal(L,{signal:K,advice:n.Signal.Handler.advices.before,args:J})
},afterTrapper:function(L,K,J){n.Signal.signal(L,{signal:K,advice:n.Signal.Handler.advices.after,args:J})
},beforePropTrapper:function(L,K,J){n.Signal.signal(L,{signal:K,advice:n.Signal.Handler.advices.propBefore,args:J})
},afterPropTrapper:function(L,K,J){n.Signal.signal(L,{signal:K,advice:n.Signal.Handler.advices.propAfter,args:J})
}}),Observer:function(K,N){var M=this,J;if(!this.PotInternal.serial){this.PotInternal.serial=k(this)
}j(this.PotInternal,{orgEvent:N||(typeof window==="object"&&window.event)||{},object:K});
J=this.PotInternal.orgEvent;if(!n.isObject(J)){this.PotInternal.orgEvent=J={type:J}
}d(J,function(O,P){if(P){if(!(P in M)){M[P]=O}}});try{if(!J.target){J.target=J.srcElement||n.currentDocument()||{}
}if(J.target.nodeType==3&&J.target.parentNode){J.target=J.target.parentNode}if(J.metaKey==null){J.metaKey=J.ctrlKey
}if(J.timeStamp==null){J.timeStamp=b()}if(J.relatedTarget==null){if(/mouse(?:over|enter)/.test(J.type)){J.relatedTarget=J.fromElement
}else{if(/mouse(?:out|leave)/.test(J.type)){J.relatedTarget=J.toElement}}}}catch(L){}},handlers:[],locked:false,hasDebris:false,isHandler:function(J){return J!=null&&((J instanceof n.Signal.Handler)||(J.id!=null&&J.id===n.Signal.Handler.prototype.id&&J.NAME!=null&&J.NAME===n.Signal.Handler.prototype.NAME))
},isObserver:function(J){return J!=null&&((J instanceof n.Signal.Observer)||(J.PotInternal!=null&&J.PotInternal.id!=null&&J.PotInternal.id===n.Signal.Observer.prototype.PotInternal.id&&J.PotInternal.NAME!=null&&J.PotInternal.NAME===n.Signal.Observer.prototype.PotInternal.NAME))
},attach:function(N,Q,T,L,J){var O=[],K,R,P,S,M;K=E(N);if(!K){return false}R=C(K);
S=!!L;if(n.isArray(Q)){P=true}M=n.Signal.Handler.advices.normal;d(A(Q),function(X){var U,V,W;
U=t(X);W=I(K,U,T,S,R,J,M);V=new n.Signal.Handler({object:K,signal:U,listener:W,callback:T,isDOM:R,useCapture:S,advice:M,attached:true});
n.Signal.handlers.push(V);if(R){if(K.addEventListener){K.addEventListener(F(K,U),W,S)
}else{if(K.attachEvent){K.attachEvent(F(K,U),W)}}}O[O.length]=V});return P?O:O[0]
},attachBefore:function(L,K,N,J,M){return H(L,K,N,n.Signal.Handler.advices.before,M)
},attachAfter:function(L,K,N,J,M){return H(L,K,N,n.Signal.Handler.advices.after,M)
},attachPropBefore:function(K,M,N,J,L){return D(K,M,N,n.Signal.Handler.advices.propBefore,L)
},attachPropAfter:function(K,M,N,J,L){return D(K,M,N,n.Signal.Handler.advices.propAfter,L)
},detach:function(M,R,S,L){var T=false,Q=arguments,J=n.Signal,P=-1,N,K,O;K=E(M);if(!K){return false
}if(J.isHandler(K)){d(J.handlers,function(V,U){if(V&&V===K&&(!R||V.signal===R)&&(!S||V.callback===S)){P=U;
throw n.StopIteration}});if(~P){G(K);if(!J.locked){J.handlers.splice(P,1)}else{J.hasDebris=true
}T=true}}else{if(Q.length>1){for(N=J.handlers.length-1;N>=0;N--){O=J.handlers[N];
if(O&&O.object===K&&O.signal===R&&O.callback===S){G(O);if(!J.locked){J.handlers.splice(N,1)
}else{J.hasDebris=true}T=true;break}}}}return T},detachAll:function(){var P=n.Signal,K=arguments,O,J=[],L,M,N={};
switch(K.length){case 0:break;case 1:O=K[0];break;case 2:O=K[0];J=K[1]||[];break;
default:O=K[0];J=A(K,1);break}if(O==null){O=n.Global}else{O=E(O)}if(!O){return false
}J=A(J);d(J,function(Q){N[t(Q)]=true});for(L=P.handlers.length-1;L>=0;L--){M=P.handlers[L];
if(!M||(M.object===O&&((!J||J.length===0)||(J.length&&M.signal in N)))){if(M){G(M)
}if(!P.locked){P.handlers.splice(L,1)}else{P.hasDebris=true}}}},signal:function(N,R){var O=[],Q=A(arguments,2),S=[],P,K,J=n.Signal,T,L,M={};
K=E(N);if(!K){return false}if(n.isObject(R)&&R.signal!=null){T=R.signal;L=R.advice;
Q=A(R.args)}else{T=R;L=n.Signal.Handler.advices.normal}d(A(T),function(U){M[t(U)]=true
});J.locked=true;d(J.handlers,function(U){if(U&&U.attached&&U.advice===L&&U.object===K&&U.signal in M){try{O[O.length]=U.listener.apply(K,Q)
}catch(V){S[S.length]=V}}});if(J.hasDebris){for(P=J.handlers.length-1;P>=0;P--){if(!J.handlers[P]||!J.handlers[P].attached){J.handlers.splice(P,1)
}}J.hasDebris=false}J.locked=false;switch(S.length){case 0:break;case 1:throw S[0];
default:throw j(S[0],{errors:S})}return O}});n.Signal.Handler.prototype=j(n.Signal.Handler.prototype,{constructor:n.Signal.Handler,id:n.Internal.getMagicNumber(),NAME:"Handler",serial:null,toString:n.toString,init:function(J){if(!this.serial){this.serial=k(this)
}j(this,J);return this}});n.Signal.Handler.prototype.init.prototype=n.Signal.Handler.prototype;
n.Signal.Observer.prototype={constructor:n.Signal.Observer,PotInternal:{id:n.Internal.getMagicNumber(),NAME:"Observer",serial:null,orgEvent:null,object:null},toString:function(){return s(this.PotInternal.NAME)
},preventDefault:function(){var J;try{J=this.PotInternal.orgEvent;if(J){if(J.preventDefault){J.preventDefault()
}else{J.returnValue=false}}}catch(K){}},stopPropagation:function(){var J;try{J=this.PotInternal.orgEvent;
if(J){if(J.stopPropagation){J.stopPropagation()}J.cancelBubble=true}}catch(K){}}};
d({attach:4,attachBefore:4,attachAfter:4,attachPropBefore:4,attachPropAfter:4},function(K,J){j(n.Signal[J],{once:function(){var L=A(arguments);
L[K]=true;return n.Signal[J].apply(n.Signal,L)}})});function I(N,S,P,K,O,J,L){var M,R=/^(?:on|)(?:(?:un|)load|DOMContentLoaded)$/,Q=(O&&R.test(S));
if(J||Q){M=function(T){n.Signal.detach(N,S,P,K)}}if(!O){return function(){var V=[],T=arguments;
try{n.Signal.Handler.beforeTrapper(N,S,T)}catch(U){V[V.length]=U}try{P.apply(this,T)
}catch(U){V[V.length]=U}try{n.Signal.Handler.afterTrapper(N,S,T)}catch(U){V[V.length]=U
}if(J){M(T.callee)}if(V&&V.length){if(V.length>1){throw j(V[0],{errors:V})}else{throw V[0]
}}}}if(Q){return function(U){var W=[],T=new n.Signal.Observer(N,U);try{n.Signal.Handler.beforeTrapper(N,S,T,true)
}catch(V){W[W.length]=V}try{P.call(N,T)}catch(V){W[W.length]=V}try{n.Signal.Handler.afterTrapper(N,S,T,true)
}catch(V){W[W.length]=V}M(arguments.callee);if(W&&W.length){if(W.length>1){throw j(W[0],{errors:W})
}else{throw W[0]}}}}else{return function(T){var W=[],V=new n.Signal.Observer(N,T);
try{n.Signal.Handler.beforeTrapper(N,S,V,true)}catch(U){W[W.length]=U}try{P.call(N,V)
}catch(U){W[W.length]=U}try{n.Signal.Handler.afterTrapper(N,S,V,true)}catch(U){W[W.length]=U
}if(J){M(arguments.callee)}if(W&&W.length){if(W.length>1){throw j(W[0],{errors:W})
}else{throw W[0]}}}}}function G(L){var K,N,M,J;if(!L||!L.attached){return}L.attached=false;
if(L.isDOM&&L.advice===n.Signal.Handler.advices.normal){K=L.object;N=L.signal;J=L.useCapture;
M=L.listener;try{if(K.removeEventListener){K.removeEventListener(F(K,N),M,J)}else{if(K.detachEvent){K.detachEvent(F(K,N),M)
}}}catch(O){}}}function H(M,P,R,L,J){var N=[],K,Q,O;K=E(M);if(!K){return false}Q=C(K);
if(n.isArray(P)){O=true}d(A(P),function(U){var S,T;S=t(U);T=new n.Signal.Handler({object:K,signal:S,listener:R,callback:R,isDOM:Q,useCapture:false,advice:L,attached:true});
n.Signal.handlers.push(T);N[N.length]=T});return O?N:N[0]}function D(M,Q,X,L,K){var N=[],O,R,P,U,V,S=".",T={},W,J=n.Signal;
if(!M||!n.isFunction(X)){return false}if(n.isArray(Q)){P=true}V=function(Y){return function(){var Z=arguments,aa=Z.callee;
X.apply(M,Z);if(K){n.Signal.detach(M,Y,aa,false)}}};U=A(Q);d(U,function(Y){T[S+t(Y)]=true
});for(O=J.handlers.length-1;O>=0;O--){R=J.handlers[O];if(R&&R.object===M&&(R.advice===n.Signal.Handler.advices.propBefore||R.advice===n.Signal.Handler.advices.propAfter)&&(S+R.signal) in T){W=true;
break}}if(!W){d(U,function(Z){var Y=t(Z);n.override(M,Y,function(ad,ab){var aa,ae=[];
try{n.Signal.Handler.beforePropTrapper(M,Y,ab)}catch(ac){ae[ae.length]=ac}try{aa=ad(ab)
}catch(ac){ae[ae.length]=ac}try{n.Signal.Handler.afterPropTrapper(M,Y,ab)}catch(ac){ae[ae.length]=ac
}if(ae&&ae.length){if(ae.length>1){throw j(ae[0],{errors:ae})}else{throw ae[0]}}return aa
})})}d(U,function(aa){var ab,Y,Z;ab=t(aa);Z=V(ab);Y=new n.Signal.Handler({object:M,signal:ab,listener:Z,callback:Z,isDOM:false,useCapture:false,advice:L,attached:true});
n.Signal.handlers.push(Y);N[N.length]=Y});return P?N:N[0]}function F(J,M){var K=t(M),L="on";
if(J){if(J.addEventListener){if(K.indexOf(L)===0){K=K.substring(2)}}else{if(J.attachEvent){if(K.indexOf(L)!==0){K=L+K
}}}}return K}function C(J){return !!(J&&(J.addEventListener&&J.removeEventListener)||(J.attachEvent&&J.detachEvent))
}function E(K){if(typeof K==="object"||n.isFunction(K)){return K}if(n.isString(K)){try{return n.currentDocument().getElementById(t(K).replace(/^#/,""))
}catch(J){}}return false}n.update({attach:n.Signal.attach,attachBefore:n.Signal.attachBefore,attachAfter:n.Signal.attachAfter,attachPropBefore:n.Signal.attachPropBefore,attachPropAfter:n.Signal.attachPropAfter,detach:n.Signal.detach,detachAll:n.Signal.detachAll,signal:n.Signal.signal})
