 ...
 */
(function(c,g){var o={VERSION:"1.24",TYPE:"lite"},i=Array.prototype.push,s=Array.prototype.slice,f=Array.prototype.concat,w=Object.prototype.toString,m=Object.prototype.hasOwnProperty,n=/List|Collection/i,e=/^[\s\u00A0\u3000]+|[\s\u00A0\u3000]+$/g,p=/([-.*+?^${}()|[\]\/\\])/g,y=/^(?:[a-zA-Z0-9_~.-]|%[0-9a-fA-F]{2})*$/,q,v,k,h;
...
D[D.length]=E(K)+I+E(N)}})}return D.join("&")},parseFromQueryString:function(I,H){var D=[],G,F,E;
if(o.isObject(I)||o.isArray(I)){return I}if(H){D={}}F=u(I,true);if(F){G=o.URI.urlDecode;
E=/&(?:(?:amp|#(?:0*38|[xX]0*26));|)/;while(F.charAt(0)==="?"){F=F.substring(1)}d(F.split(E),function(K){var J,M,L;
L=K.split("=");switch(L.length){case 0:break;case 1:M=L[0];break;default:J=L[0];M=L[1];
break}if(J||M){J=u(G(J));M=u(G(M));if(H){D[J]=M}else{D[D.length]=[J,M]}}})}return D
...
ab=this.PotInternal.orgEvent;if(!o.isObject(ab)){this.PotInternal.orgEvent=ab={type:ab}
}d(ab,function(ag,ah){if(ah){if(!(ah in ae)){ae[ah]=ag}}});try{if(!ab.target){ab.target=ab.srcElement||o.currentDocument()||{}
}if(ab.target.nodeType==3&&ab.target.parentNode){ab.target=ab.target.parentNode}if(ab.metaKey==null){ab.metaKey=ab.ctrlKey
}if(ab.timeStamp==null){ab.timeStamp=b()}if(ab.relatedTarget==null){if(P.MOUSE_OVER.test(ab.type)){ab.relatedTarget=ab.fromElement
...
}return ak}).begin()}});o.Signal.Handler.prototype=j(o.Signal.Handler.prototype,{constructor:o.Signal.Handler,id:o.Internal.getMagicNumber(),NAME:"Handler",serial:null,toString:o.toString,init:function(ab){if(!this.serial){this.serial=l(this)
