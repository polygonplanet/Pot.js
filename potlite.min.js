 ...
 */
(function(c,g){var m={VERSION:"1.10",TYPE:"lite"},q=Array.prototype.slice,f=Array.prototype.concat,u=Object.prototype.toString,l=/List|Collection/i,e=/^[\s\u00A0\u3000]+|[\s\u00A0\u3000]+$/g,n=/([-.*+?^${}()|[\]\/\\])/g,o,t,i,h;
...
}}function p(G){var A=arguments,D=A.callee,C,E,B;try{if(!D.firebug("log",A)){if(!m.XPCOM.isEnabled){throw false
}B=t["@mozilla.org/consoleservice;1"].getService(o.nsIConsoleService);B.logStringMessage(String(G))
}}catch(F){if(typeof GM_log!=="undefined"){C=GM_log}else{if(typeof console!=="undefined"){C=console.debug||console.dir||console.log
}else{if(typeof opera!=="undefined"&&opera.postError){C=opera.postError}else{C=function(H){throw H
}}}}try{if(C.apply){C.apply(C,A)}else{throw C}}catch(F){try{C(G)}catch(F){try{console.log(G)
}catch(F){try{D.divConsole(G)}catch(F){}}}}}return G}j(p,{firebug:function(G,C){var A=false,E,B;
...
}else{return B}}})},maybeDeferred:function(B){var A;if(m.isDeferred(B)){A=B}else{A=m.Deferred.succeed(B)
