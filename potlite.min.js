 ...
 */
(function(c,g){var m={VERSION:"1.14",TYPE:"lite"},q=Array.prototype.slice,f=Array.prototype.concat,u=Object.prototype.toString,l=/List|Collection/i,e=/^[\s\u00A0\u3000]+|[\s\u00A0\u3000]+$/g,n=/([-.*+?^${}()|[\]\/\\])/g,w=/^(?:[a-zA-Z0-9_~.-]|%[0-9a-fA-F]{2})*$/,o,t,i,h;
...
return C.INITIAL_NUMBER+(C.count++)},{count:0,INITIAL_NUMBER:Number("0xC26BEB642C0A")||(2147483647^2047)}),getExportObject:function(D){var C;
if(D){if(m.System.isNonBrowser){C=m.Global||c}else{C=(m.isWindow(c)&&c)||(m.isWindow(m.Global)&&m.Global)
}if(!C&&typeof window!=="undefined"&&m.isWindow(window)){C=window}}if(!C){if(m.System.isNodeJS){if(typeof module==="object"&&typeof module.exports==="object"){C=module.exports
...
}if(!D.isFirefoxExtension){if(m.Browser.chrome||m.Browser.webkit||m.Browser.safari){if(typeof chrome==="object"&&typeof chrome.extension==="object"){D.isChromeExtension=true
...
}function z(D,C){var I,F,B,G=arguments.callee,E;if(G.canNodeList==null){if(c||m.Global){try{E=q.call(((c&&c.documentElement)||(m.Global&&m.Global.documentElement)).childNodes)[0].nodeType;
E=null;G.canNodeList=true}catch(H){G.canNodeList=false}}}if(D==null){I=[D]}else{switch(m.typeOf(D)){case"array":I=D.slice();
break;case"object":if(m.isArrayLike(D)){if(!G.canNodeList&&m.isNodeList(D)){I=[];
F=0;B=D.length;do{I[F]=D[F]}while(++F<B)}else{I=q.call(D)}break}default:I=q.call(f.call([],D));
break}}if(C>0){I=I.slice(C)}return I}function b(){return(new Date()).getTime()}function s(D,E){var C="",F,B=arguments.length;
...
}).then(function(){var E,D,F,G;E={borderWidth:"1px",borderStyle:"solid",borderColor:"#999",background:"#fff",color:"#333",fontSize:"13px",fontFamily:"monospace",position:"absolute",padding:"10px",margin:"0px"};
G=document.createElement("div");D=G.style;d(E,function(I,H){D[H]=I});D.borderWidth="3px";
D.position="fixed";D.width="95%";D.height="20%";D.paddingRight="15px";D.zIndex=9996;
D.left="0px";D.bottom="0px";B.ieConsole=G.cloneNode(false);B.ieConsole.id=B.ieConsoleId=k(m,"");
D=B.ieConsole.style;D.borderWidth="1px";D.width="97%";D.height="87%";D.position="relative";
D.zIndex=9997;D.padding="5px";D.whiteSpace="pre";D.wordWrap="break-word";D.overflowX="hidden";
D.overflowY="auto";B.hr=document.createElement("hr");D=B.hr.style;D.position="static";
D.width="100%";D.border="1px solid #aaa";D.zIndex=9998;F=document.createElement("div");
D=F.style;d(E,function(I,H){D[H]=I});D.zIndex=9999;D.fontFamily="sans-serif";D.fontWeight="bold";
D.borderWidth="1px";D.padding="1px 4px";D.lineHeight=1;D.right="2px";D.top="2px";
D.cursor="pointer";F.title="close";F.innerHTML="x";F.onclick=function(){G.parentNode.removeChild(G);
G=B.ieConsole=F.onclick=null};G.appendChild(F);G.appendChild(B.ieConsole);document.body.appendChild(G);
...
},B+(b()-H))}else{E.begin()}return E})},parallel:function(G){var B,D=arguments,H,F,C,E;
if(D.length===0){B=m.Deferred.succeed()}else{if(D.length===1){if(m.isObject(G)){F=G
}else{F=z(G)}}else{F=z(D)}B=new m.Deferred({canceller:function(){d(F,function(I){if(m.isDeferred(I)){I.cancel()
}})}});H=new m.Deferred();E=[];C=m.isObject(F)?{}:[];d(F,function(I,J){var K;if(m.isDeferred(I)){K=I
}else{if(m.isFunction(I)){K=new m.Deferred();K.then(function(){var L=I();if(m.isDeferred(L)&&L.state===m.Deferred.states.unfired){L.begin()
}return L})}else{K=m.Deferred.succeed(I)}}if(!m.isDeferred(K)){K=m.Deferred.maybeDeferred(K)
}E[E.length]=J;H.then(function(){if(K.state===m.Deferred.states.unfired){m.Deferred.callLazy(K)
}K.then(function(L){C[J]=L;E.pop();if(E.length===0){B.begin(C)}},function(L){B.raise(L)
})})});m.Deferred.callLazy(H)}return B},chain:function(){var C=arguments,B=C.length,F,D,E;
D=new m.Deferred();if(B>0){F=z(B===1?C[0]:C);E={name:/^\s*[()]*\s*function\s*([^\s()]+)/,rescue:/rescue|raise|err|fail/i};
d(F,function(I){var G;if(m.isFunction(I)){try{G=I.toString().match(E.name)[1]}catch(H){}if(G&&E.rescue.test(G)){D.rescue(I)
...
}catch(G){I=G;H.raise(I)}return H}})(jQuery)}})()});m.Internal.exportPot(false,false,true)
})(this||{});
